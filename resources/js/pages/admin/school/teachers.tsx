...
export default function TeachersPage() {
    ...
    function load(p: number = 1, s: string = '') {
        ...
        if (s) params.set('q', s);
        fetchJson(`/api/admin/teachers?${params.toString()}`)
            .then((json) => {
            ...
            <div className="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
                <div className="flex justify-between items-center">
                    <div>
                        ...
                        <p className="text-sm text-gray-600 dark:text-gray-400">Manage teachers and their assignments</p>
                    </div>
                    <Link href="/admin/school/teachers/create"><Button className="flex items-center gap-2"><Plus className="h-4 w-4"/>Add Teacher</Button></Link>
                </div>
                ...
                <div className="flex gap-4 items-end">
                    <form onSubmit={handleSearch} className="flex gap-2 flex-1">
                        <div className="relative flex-1">
                            ...
                            <Input type="text" placeholder="Search teachers by name or email..." value={search} onChange={(e) => setSearch((e.target as HTMLInputElement).value)} className="pl-10" />
                        </div>
                                ...
                                {data.length > 0 ? (
                                    data.map((it) => (
                                        <tr key={it.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{it.phone}</td>
                                            <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                <div className="flex gap-2 justify-end">
                                                    <Link href={`/admin/school/teachers/${it.id}`}><Button variant="outline" size="sm"><Eye className="h-4 w-4"/></Button></Link>
                                                    <Link href={`/admin/school/teachers/${it.id}/edit`}><Button variant="outline" size="sm"><Edit className="h-4 w-4"/></Button></Link>
                                                </div>
                                            </td>
                                        </tr>
                                    ))
                                    ...
                                    <tr>
                                        <td colSpan={4} className="px-6 py-12 text-center">
                                            <div className="text-gray-500 dark:text-gray-400">No teachers found.</div>
                                            <Link href="/admin/school/teachers/create" className="mt-2 inline-block"><Button>Add your first teacher</Button></Link>
                                        </td>
                                    </tr>
                                )}
                            </tbody>
