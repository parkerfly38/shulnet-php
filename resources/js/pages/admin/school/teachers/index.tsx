...
export default function TeachersIndex() {
    ...
    function handleDelete(id: number, name: string) {
        ...
        fetchJson(`/api/admin/teachers/${id}`, { method: 'DELETE' })
    ...
    return (
        <AppLayout breadcrumbs={breadcrums} >
            <Head title="Teachers - School Management" />
            <div className="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
                <div className="flex justify-between items-center">
                    <div>
                        <h1 className="text-2xl font-semibold text-gray-900 dark:text-gray-100">Teachers</h1>
                        <p className="text-sm text-gray-600 dark:text-gray-400">Manage teaching staff and faculty</p>
                    </div>
                    <Link href="/admin/school/teachers/create">
                ...
                <div className="flex gap-4 items-end">
                    <form onSubmit={handleSearch} className="flex gap-2 flex-1">
                        <div className="relative flex-1">
                            ...
                            <Input type="text" placeholder="Search teachers by name..." value={search} onChange={(e) => setSearch((e.target as HTMLInputElement).value)} className="pl-10" />
                        </div>
                                ...
                                {data.length > 0 ? (
                                    data.map((it, idx) => (
                                        <tr key={it.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{it.position_title}</td>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                <div className="flex gap-2 justify-end">
                                                    <Link href={`/admin/school/teachers/${it.id}`}>
                                                        <Button variant="outline" size="sm"><Eye className="h-4 w-4" /></Button>
                                                    </Link>
                                                    <Link href={`/admin/school/teachers/${it.id}/edit`}>
                                                        <Button variant="outline" size="sm"><Edit className="h-4 w-4" /></Button>
                                                    </Link>
                                                    <Button 
                                                        variant="outline" 
                                                        size="sm" 
                                                        onClick={() => handleDelete(it.id, `${it.title} ${it.first_name} ${it.last_name}`)}
