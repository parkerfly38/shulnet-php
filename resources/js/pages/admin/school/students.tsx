...
export default function StudentsPage() {
    ...
    function updateUrl(p: number, s: string) {
        ...
        const url = q ? `/admin/school/students?${q}` : '/admin/school/students';
    ...
    function load(p: number = 1, s: string = '') {
        ...
        fetchJson(`/api/admin/students?${params.toString()}`)
            ...
            .catch(() => {
                setData([]); setTotal(0); setFrom(0); setTo(0); setLastPage(1);
            })
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
                <div className="flex justify-between items-center">
                    <div>
                        <h1 className="text-2xl font-semibold text-gray-900 dark:text-gray-100">Student Management</h1>
                        <p className="text-sm text-gray-600 dark:text-gray-400">Manage students and enrollment</p>
                    </div>
                    <Link href="/admin/school/students/create">
                        <Button className="flex items-center gap-2"><Plus className="h-4 w-4"/>Add Student</Button>
                    </Link>
                </div>
                ...
                <div className="flex gap-4 items-end">
                    <form onSubmit={handleSearch} className="flex gap-2 flex-1">
                        <div className="relative flex-1">
                            ...
                            <Input type="text" placeholder="Search students by name, parent, or email..." value={search} onChange={(e) => setSearch((e.target as HTMLInputElement).value)} className="pl-10" />
                        </div>
                                ...
                                {data.length > 0 ? (
                                    data.map((it) => (
                                        <tr key={it.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-gray-100">{it.first_name} {it.last_name}</td>
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{it.dob ?? it.date_of_birth}</td>
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{it.parent?.first_name ? `${it.parent.first_name} ${it.parent.last_name}` : it.parent_id}</td>
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{it.email}</td>
                                            <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                <div className="flex gap-2 justify-end">
                                                    <Link href={`/admin/school/students/${it.id}`}><Button variant="outline" size="sm"><Eye className="h-4 w-4"/></Button></Link>
                                                    <Link href={`/admin/school/students/${it.id}/edit`}><Button variant="outline" size="sm"><Edit className="h-4 w-4"/></Button></Link>
                                                </div>
                                            </td>
                                        </tr>
                                    ))
