...
export default function GravesitesCreate({ members }: Readonly<Props>) {
    const { data, setData, post, processing, errors } = useForm({
        ...
        gravesite_type: 'single' as 'single' | 'double' | 'family' | 'cremation',
        size_length: '',
        size_width: '',
        member_id: '',
        purchase_date: '',
    ...
    const breadcrumbs: BreadcrumbItem[] = useMemo(() => [
        ...
        { title: 'Create Gravesite', href: '/admin/gravesites/create' },
    ], []);

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post('/admin/gravesites');
    };

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Create Gravesite" />

            <div className="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Create Gravesite</h1>
                        <p className="text-gray-600 dark:text-gray-400">Add a new cemetery gravesite</p>
                    </div>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="flex justify-end gap-4">
                        ...
                        <Button type="submit" disabled={processing}>
                            {processing ? 'Creating...' : 'Create Gravesite'}
                        </Button>
                    </div>
                </form>
            </div>
