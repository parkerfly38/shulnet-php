...
import { type MembershipPeriod, type BreadcrumbItem } from '@/types';
...
export default function MembershipPeriodsEdit({ member, membershipPeriod, invoices }: Readonly<Props>) {
  const { data, setData, put, processing, errors } = useForm<MembershipPeriodForm>({
    begin_date: membershipPeriod.begin_date || '',
    end_date: membershipPeriod.end_date || '',
    membership_type: membershipPeriod.membership_type || 'annual',
    invoice_id: membershipPeriod.invoice_id?.toString() || '',
    notes: membershipPeriod.notes || '',
  });
  ...
  const breadcrumbs: BreadcrumbItem[] = useMemo(() => [
    ...
    },
    {
      ...
      href: `/admin/members/${member.id}/membership-periods/${membershipPeriod.id}/edit`,
    }
  ], [member, membershipPeriod.id]);

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    put(`/admin/members/${member.id}/membership-periods/${membershipPeriod.id}`);
  };

  return (
    <AppLayout breadcrumbs={breadcrumbs}>
      <Head title={`Edit Membership Period - ${member.first_name} ${member.last_name}`} />
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">
              Update membership period for {member.first_name} {member.last_name}
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex items-center justify-end gap-4">
            ...
            <Button type="submit" disabled={processing}>
              ...
              {processing ? 'Saving...' : 'Update Membership Period'}
            </Button>
          </div>
        </form>
      </div>
